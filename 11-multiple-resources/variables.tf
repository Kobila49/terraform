...
# Commented out because of last lession in  this section
...
variable "ec2_instance_config_list" {
  ...
  # Default is empty because of last lession in  this section
  default = []

  # Ensure that only t3.micro is used
  validation {
    condition     = alltrue([for config in var.ec2_instance_config_list : contains(["t3.micro"], config.instance_type)])
    error_message = "Only t3.micro instances are allowed."
  }

  # Ensure that only ubuntu images is used
  validation {
    condition = alltrue([
...
variable "ec2_instance_config_map" {
  ...
  # Ensure that only t3.micro is used
  validation {
    condition     = alltrue([for config in values(var.ec2_instance_config_map) : contains(["t3.micro"], config.instance_type)])
    error_message = "Only t3.micro instances are allowed."
  }
